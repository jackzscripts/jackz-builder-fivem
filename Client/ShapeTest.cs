...
namespace jackz_builder.Client
{
    ...
    public class RayResult
    {
        ...
        public RayResult(bool didHit, Vector3 coords, Vector3 surfaceNormal, int entity, uint materialHash)
        {
            DidHit = didHit;
            Coords = coords;
            SurfaceNormal = surfaceNormal;
            Entity = Entity.FromHandle(entity);
            MaterialHash = (MaterialHash) materialHash;
        }
    }
    ...
    public class RayHandle
    {
        ...
        public bool Check()
        {
            ...
            int entityHit = 0;
            uint materialHash = 0;
            var result = GetShapeTestResultIncludingMaterial(Handle, ref hit, ref endCoords, ref surfaceNormal, ref materialHash, ref entityHit);
            if (result == 0)
            {
                // Handle is invalid
            ...
            } else if (result == 2)
            {
                ...
                Result = new RayResult(hit, endCoords, surfaceNormal, entityHit, materialHash);
                return true;
            }
    ...
    public static class ShapeTest
    {
        ...
        public static RayHandle Box(Vector3 from, Vector3 to, Vector3 rotation, TraceFlags flags, int ignoreEntity = 0)
        {
            var handle = StartShapeTestBox(from.X, from.Y, from.Z, to.X, to.Y, to.Z, rotation.X, rotation.Y, rotation.Z, 0, (int) flags,
                ignoreEntity, 4);
            return new RayHandle(handle);
        }
        public static RayHandle Box(Entity entity, Vector3 to, Vector3 rotation, TraceFlags flags, int ignoreEntity = 0)
        {
            return Box(entity.Position, to, rotation, flags, ignoreEntity != 0 ? ignoreEntity : entity.Handle);
        }
